...
Uses the best skeleton model trained on MediaPipe landmark drawings.
"""
...
class Config:
    ...
    MODEL_PATH = "best_skeleton_model.h5"           # <-- updated
    CLASS_NAMES_PATH = "skeleton_class_names.txt"    # <-- updated
...
class GestureRecognitionApp:
    """Main application for real-time hand gesture recognition (skeleton model)."""
    ...
    def __init__(self, config: Config):
        ...
        self.prev_crop_gray = None   # used for movement detection on original frame (optional)
        self.last_move_time = time.time()
        self.last_prediction_time = time.time()
        self.frame_count = 0
        self.prediction_queue = deque(maxlen=config.PREDICTION_BUFFER)

        logger.info("Initializing Hand Gesture Recognition (Skeleton Model)")
    ...
    def process_frame(self, frame: np.ndarray) -> Tuple[Optional[str], float]:
        ...
        try:
            ...
            # (Optional) use grayscale version of skeleton for movement detection?
            # We'll keep movement detection on the original frame's crop for simplicity,
            # but we could also use skeleton. For now we'll just use a copy of the original crop
            # to maintain the movement detection logic. Let's reuse the earlier crop logic
            # only for movement detection, but we need a grayscale version of the hand region.
            # We'll keep the previous approach: use crop of original frame for movement.
            # However, the code originally used crop for movement, so we'll keep that part.

            # For movement detection, we'll use a grayscale crop from the original frame
            h, w, _ = frame.shape
            ...
            # Only predict if hand is stable
            if movement < self.config.MOVEMENT_THRESHOLD:
                ...
                if (time_stable > self.config.STABLE_DURATION and
                    time_since_last_pred > self.config.PREDICTION_INTERVAL):

                    predicted_class, confidence, all_preds = predict_gesture(
                        self.model,
                        skeleton,                     # <-- use skeleton image
                        self.class_names,
                    ...
                    if self.config.SAVE_SKELETONS and smoothed_pred != "Unknown":
                        save_skeleton(skeleton, smoothed_pred, self.config.SAVE_DIR)

                    if smoothed_pred != "Unknown":
                        logger.info(f"✋ Predicted: {smoothed_pred} (confidence: {confidence:.2f})")
            ...
            # Show the generated skeleton in a small window (optional)
